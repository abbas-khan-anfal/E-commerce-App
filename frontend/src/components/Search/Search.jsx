...
function Search() {
    ...
    const fetchProductsHanlder = async (search) => {
        setLoading(true);
        try
        {
          const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/product/searchproduct/${search}`, {
            withCredentials : true
          });
          setProducts(response.data.searchedProducts);
        }
        catch(error)
        {
      ...
      useEffect(() => {
        fetchProductsHanlder(searchterm);
        ...
        <div className='container'>
            <div className="row">
                ...
                <div className="col-12 mb-4">
                    <Title title={`Search for : ${searchterm}`} />
                    <span className='text-secondary'>{products.length} Result found</span>
                ...
                :
                <Message message="No products found for this search term, try different keywords." />
                }
